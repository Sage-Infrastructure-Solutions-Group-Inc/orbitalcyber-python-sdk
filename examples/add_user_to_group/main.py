...
parser = ArgumentParser(description='basic example of how to add a user to a group using the OrbitalCyber API.')
...
parser.add_argument('--user-id', help="the GUID of the user account", required=True)
parser.add_argument('--group-id', help="the GUID of the group", required=True)
...
add_members_resp = client.post(f"/api/auth/company-mgmt/group/{args.group_id}/members/add", json={'users': [args.user_id]})
print(add_members_resp.content)
